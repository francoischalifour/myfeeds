  ...
  render () {
    ...
    return (
      <Sidebar>
        ...
        <Content>
          <Name><Link to={`/@${username}`}>{name}</Link></Name>
          <Username><Link to={`/@${username}`}>@{username}</Link></Username>

          {description &&
            <Description dangerouslySetInnerHTML={{ __html: formatText(description)}} />
          }
          ...
          {getCurrentUserId() !== userId && (
            !this.state.isFollowing
              ? <button className='button' onClick={() => this.handleFollow(true)}>Follow</button>
              : <button className='button outline' onClick={() => this.handleFollow(false)}>Unfollow</button>
          )}
        </Content>
      </Sidebar>
    )
